 ...
 */
abstract class BaseZone extends BaseObject  implements Persistent {
	 ...
	 * @return     Zone The current object (for fluent API support)
	 */
	public function setIdZone($v)
	{
		...
		if ($this->id_zone !== $v) {
			$this->id_zone = $v;
			$this->modifiedColumns[] = ZonePeer::ID_ZONE;
		}
	...
	public function setZone($v)
	{
		...
		if ($this->zone !== $v) {
			$this->zone = $v;
			$this->modifiedColumns[] = ZonePeer::ZONE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->zone = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = ZonePeer::NUM_COLUMNS - ZonePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Zone object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ZonePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseZone:delete:pre') as $callable)
			{
			...
			if ($ret) {
				ZonePeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseZone:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseZone:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseZone:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				ZonePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$this->setIdZone($pk);  //[IMV] update autoincrement primary key
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = ZonePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = ZonePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = ZonePeer::getFieldNames($keyType);
		$result = array(
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setZone($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ZonePeer::ID_ZONE)) $criteria->add(ZonePeer::ID_ZONE, $this->id_zone);
		if ($this->isColumnModified(ZonePeer::ZONE)) $criteria->add(ZonePeer::ZONE, $this->zone);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(ZonePeer::ID_ZONE, $this->id_zone);
	 ...
	 * Otherwise if this Zone has previously been saved, it will retrieve
	 * related SfGuardUserProfiles from storage. If this Zone is new, it will return
		...
		if ($this->collSfGuardUserProfiles === null) {
			if ($this->isNew()) {
			   $this->collSfGuardUserProfiles = array();
				...
				$criteria->add(SfGuardUserProfilePeer::ID_ZONE, $this->id_zone);
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseZone:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseZone::%s', $method));
	  }
