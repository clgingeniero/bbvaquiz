...
abstract class BasePosition extends BaseObject  implements Persistent {
	...
	public function setIdPosition($v)
	{
		...
		if ($this->id_position !== $v) {
			$this->id_position = $v;
			$this->modifiedColumns[] = PositionPeer::ID_POSITION;
		}
	...
	public function setPosition($v)
	{
		...
		if ($this->position !== $v) {
			$this->position = $v;
			$this->modifiedColumns[] = PositionPeer::POSITION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->id_position = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			...
			return $startcol + 2; // 2 = PositionPeer::NUM_COLUMNS - PositionPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BasePosition:delete:pre') as $callable)
			{
			...
			if ($ret) {
				PositionPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BasePosition:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BasePosition:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BasePosition:save:post') as $callable)
				{
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = PositionPeer::doInsert($this, $con);
					$affectedRows += 1; // we are assuming that there is only 1 row per doInsert() which
										 // should always be true here (even though technically
										 // BasePeer::doInsert() can insert multiple rows).

					$this->setIdPosition($pk);  //[IMV] update autoincrement primary key

					$this->setNew(false);
				} else {
					$affectedRows += PositionPeer::doUpdate($this, $con);
				}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setIdPosition($arr[$keys[0]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PositionPeer::ID_POSITION)) $criteria->add(PositionPeer::ID_POSITION, $this->id_position);
		if ($this->isColumnModified(PositionPeer::POSITION)) $criteria->add(PositionPeer::POSITION, $this->position);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PositionPeer::ID_POSITION, $this->id_position);
	...
	/**
	 * Generic method to set the primary key (id_position column).
	 *
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setIdPosition(NULL); // this is a auto-increment column, so set to default value
	 ...
	 * Otherwise if this Position has previously been saved, it will retrieve
	 * related SfGuardUserProfiles from storage. If this Position is new, it will return
		...
		if ($this->collSfGuardUserProfiles === null) {
			if ($this->isNew()) {
			   $this->collSfGuardUserProfiles = array();
				...
				$criteria->add(SfGuardUserProfilePeer::ID_POSITION, $this->id_position);
	 ...
	 * actually need in Position.
	 */
	public function getSfGuardUserProfilesJoinDepto($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PositionPeer::DATABASE_NAME);
		}
		...
		if ($this->collSfGuardUserProfiles === null) {
			if ($this->isNew()) {
				...
				$this->collSfGuardUserProfiles = SfGuardUserProfilePeer::doSelectJoinDepto($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	{
	  if (!$callable = sfMixer::getCallable('BasePosition:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BasePosition::%s', $method));
	  }
